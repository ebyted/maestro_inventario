...
class ProductImporter:
    ...
    def get_or_create_category(self, category_name: str, business_id: int) -> Optional[Category]:
        ...
        if not category:
            # Generate a code for the category
            category_code = category_name.upper().replace(' ', '_').replace('/', '_').replace('-', '_')
            category_code = ''.join(c for c in category_code if c.isalnum() or c == '_')[:50]
            
            category = Category(
                business_id=business_id,
                name=category_name,
                code=category_code,
                description=f"Categoría importada desde CSV: {category_name}"
            )
    ...
    def get_or_create_brand(self, brand_name: str, business_id: int) -> Optional[Brand]:
        ...
        if not brand:
            # Generate a code for the brand
            brand_code = brand_name.upper().replace(' ', '_').replace('/', '_').replace('-', '_')
            brand_code = ''.join(c for c in brand_code if c.isalnum() or c == '_')[:50]
            
            brand = Brand(
                business_id=business_id,
                name=brand_name,
                code=brand_code,
                description=f"Marca importada desde CSV: {brand_name}"
            )
            self.db.add(brand)
            self.db.flush()
    ...
    def create_product_from_row(self, row: Dict[str, str], business: Business) -> Optional[Product]:
        ...
        try:
            ...
            if row.get('category'):
                ...
                if category:
                    logger.debug(f"Product will be linked to category: {category.name} (ID: {category.id})")
            
            brand = None
            if row.get('brand'):
                brand = self.get_or_create_brand(row['brand'], business.id)
                if brand:
                    logger.debug(f"Product will be linked to brand: {brand.name} (ID: {brand.id})")
            ...
            # Add maximum_stock only if it has a value
            if row.get('maximum_stock') and row.get('maximum_stock').strip():
                try:
                    product_data['maximum_stock'] = float(row.get('maximum_stock'))
                except ValueError:
                    logger.warning(f"Invalid maximum_stock value: {row.get('maximum_stock')}")
            
            # Remove None and empty string values, but keep 0 values
            ...
            # Log the relationships that were created
            if category:
                logger.info(f"Product '{product.name}' linked to category '{category.name}'")
            if brand:
                logger.info(f"Product '{product.name}' linked to brand '{brand.name}'")
