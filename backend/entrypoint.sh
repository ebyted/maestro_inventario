...
EXISTS=$(psql -h db -U postgres -d maestro_inventario -tAc "SELECT to_regclass('public.businesses') IS NOT NULL;")

if [ "$EXISTS" = "f" ]; then
  echo "La base de datos está vacía. Restaurando backup.sql..."
  if [ -f /app/backup.sql ]; then
    psql -h db -U postgres -d maestro_inventario -f /app/backup.sql
    alembic stamp head
  else
